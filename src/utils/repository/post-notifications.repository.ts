...
type PostNotificationEvent = (typeof postNotifications.$inferSelect)['event']
type PostNotificationInsert = typeof postNotifications.$inferInsert

/* v8 ignore start */
export async function insertPostNotifications(
  values: Array<PostNotificationInsert>,
): Promise<void> {
  if (values.length === 0) return
  const db = await getDb()
  await db.insert(postNotifications).values(values)
}
...
export async function markNotificationGroupRead(
  ...
  await db
    ...
    .where(
      and(
        eq(postNotifications.userId, userId),
        eq(postNotifications.event, event),
