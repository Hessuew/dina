...
function validateQuestionFields(
  value: z.infer<z.ZodObject<typeof questionFields>>,
  ctx: z.RefinementCtx,
) {
...
const questionWithoutExamSchema = z
  .object(questionFields)
  .superRefine(validateQuestionFields)

export const saveExamChangesSchema = z.object({
  examId: z.uuid('Invalid exam ID'),
  title: z.string().min(1, 'Title is required'),
  durationMinutes: z.number().int().positive('Duration must be positive'),
  opensAt: z.string().min(1, 'Open date is required'),
  closesAt: z.string().min(1, 'Close date is required'),
  questions: z.array(questionWithoutExamSchema),
  deletedQuestionIds: z.array(z.uuid('Invalid question ID')),
})
