  ...
  return (
    <div className="space-y-6 border-y border-white/10 py-6">
      <div className="grid gap-6 sm:grid-cols-2">
        ...
        <ProgramStat label="Teaching Faculty" value="6 Lecturer Pairs" />
        <ProgramStat
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-3 text-base leading-8 text-[#D3CAC0]">
          How to disciple others, explain beliefs clearly, and think, respond &
      ...
      <div className="border-l-2 border-[#C5A059]/40 pl-5">
        <p className="text-base leading-6 text-[#D8D0C7]">
          Each student needs to disciple at least one person during the 9 month
          period
        </p>
      </div>
    </div>
      ...
      <LandingFeaturePanelBody className="px-4 py-5 sm:px-8 sm:py-6">
        ...
        <div className="mt-5 space-y-4">
          {timeline.map((event, index) => (
            <TimelineEventRow key={event.month} event={event} index={index} />
          ))}
        </div>
      </LandingFeaturePanelBody>
    </LandingFeaturePanel>
