...
const LookupPanel = lazy(() =>
  import('./LookupPanel').then((module) => ({ default: module.LookupPanel })),
)
...
async function copyCohortEmails(
  ...
  try {
    ...
    setCopied(true)
    toast.success(resolveCopySuccessMessage('cohort'))
...
function LoadingLookupPanel() {
  return (
    <p className="flex-1 py-8 text-center text-[0.78rem] text-[#8E816D]">
      Loading contact lookup...
    </p>
  )
}
...
export function ExportContactsDialog({
  ...
  const canCopy = Boolean(cohort.emails?.length)
        ...
        <div className="relative flex min-h-0 flex-1 flex-col">
          ...
          {mode === 'cohort' && (
            <ExportEmailsFooter
              ...
              copied={cohort.copied}
              copyLabel={resolveCopyLabel('cohort', 'email')}
              onCopy={() =>
                void copyCohortEmails(cohort.emails, cohort.setCopied)
              }
