...
function ViewerMeetingTime({ anchorAt }: { anchorAt: string | null }) {
  const next = nextOccurrenceFrom(anchorAt, new Date())
  return <>{next ? format(next, 'MMM d · HH:mm') : 'Not scheduled'}</>
}
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      <div className="flex items-center gap-1.5 text-[0.72rem] text-[#6B5E4C]">
        ...
        <span>
          <ClientOnly fallback={anchorAt ? 'Loading…' : 'Not scheduled'}>
            <ViewerMeetingTime anchorAt={anchorAt} />
          </ClientOnly>
        </span>
      </div>
      {canManage && (
