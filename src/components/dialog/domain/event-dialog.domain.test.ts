...
describe('getEventDefaultValues', () => {
  ...
  it('returns blank values for edit/view mode when no event is provided', () => {
    expect(getEventDefaultValues('edit')).toEqual(getEventDefaultValues('create'))
    expect(getEventDefaultValues('view')).toEqual(getEventDefaultValues('create'))
  })
...
describe('buildEventViewModel', () => {
  ...
  it('returns a null category when the event has none', () => {
    expect(buildEventViewModel(makeEvent({ category: null })).category).toBeNull()
  })
