...
describe('resolveLessonRowView', () => {
    ...
    it('is false when lesson.isPublished is null', () => {
      expect(
        resolveLessonRowView({
          ...baseInput,
          lesson: { isPublished: null, content: 'body' },
        }).isPublished,
      ).toBe(false)
    })
  ...
  describe('indexLabel', () => {
    it('zero-pads single-digit positions (1-based)', () => {
      expect(resolveLessonRowView({ ...baseInput, index: 0 }).indexLabel).toBe(
        '01',
      )
    })

    it('renders two-digit positions without padding', () => {
      expect(resolveLessonRowView({ ...baseInput, index: 9 }).indexLabel).toBe(
        '10',
      )
    })

    it('does not truncate three-digit positions', () => {
      expect(resolveLessonRowView({ ...baseInput, index: 99 }).indexLabel).toBe(
        '100',
      )
    })
  })
  ...
  describe('showContentText', () => {
    ...
    it('is false when content is visible but empty', () => {
      expect(
        resolveLessonRowView({
          ...baseInput,
          lesson: { isPublished: true, content: '' },
        }).showContentText,
      ).toBe(false)
    })
    ...
    it('is false when content is null', () => {
      expect(
        resolveLessonRowView({
          ...baseInput,
          lesson: { isPublished: true, content: null },
        }).showContentText,
      ).toBe(false)
    })
