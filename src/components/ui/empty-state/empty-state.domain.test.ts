...
describe('getEmptyStateChrome', () => {
  describe('shouldShowAction', () => {
    ...
    it('is false when showAction is false', () => {
      expect(
        getEmptyStateChrome({ ...base, showAction: false }).shouldShowAction,
      ).toBe(false)
    })

    it('is false when actionLabel is missing', () => {
      expect(
        getEmptyStateChrome({ ...base, actionLabel: undefined })
          .shouldShowAction,
      ).toBe(false)
    })
    ...
    it('is false when there is no action handler', () => {
      expect(
        getEmptyStateChrome({ ...base, hasAction: false }).shouldShowAction,
      ).toBe(false)
    })

    it('is always a boolean, never a falsy string', () => {
      expect(
        getEmptyStateChrome({ ...base, actionLabel: '' }).shouldShowAction,
      ).toBe(false)
    })
  })
  ...
  describe('buttonTheme', () => {
    it('is dark for the dark variant', () => {
      expect(
        getEmptyStateChrome({ ...base, variant: 'dark' }).buttonTheme,
      ).toBe('dark')
    })

    it('is light for the light variant', () => {
      expect(
        getEmptyStateChrome({ ...base, variant: 'light' }).buttonTheme,
      ).toBe('light')
    })
  })
  ...
  describe('wrapperClassName', () => {
    it('uses the dashed bordered box for the light variant', () => {
      const { wrapperClassName } = getEmptyStateChrome({
        ...base,
        variant: 'light',
      })
...
describe('buildEmptyStateTextRows', () => {
  ...
  it('skips empty strings', () => {
    expect(
      buildEmptyStateTextRows({ heading: '', message: '', description: '' }),
    ).toEqual([])
  })
})
