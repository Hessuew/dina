...
describe('resolveMenuButtonTooltip', () => {
  ...
  it('shows the tooltip only when collapsed on desktop', () => {
    expect(
      resolveMenuButtonTooltip('x', { state: 'collapsed', isMobile: false })
        ?.hidden,
    ).toBe(false)
    expect(
      resolveMenuButtonTooltip('x', { state: 'expanded', isMobile: false })
        ?.hidden,
    ).toBe(true)
    expect(
      resolveMenuButtonTooltip('x', { state: 'collapsed', isMobile: true })
        ?.hidden,
    ).toBe(true)
    expect(
      resolveMenuButtonTooltip('x', { state: 'expanded', isMobile: true })
        ?.hidden,
    ).toBe(true)
  })
