...
import type { AssignmentCourse } from '@/components/view/assignments-view/assignments-view.domain'
import {
...
type AssignmentsHeaderProps = {
  role: AssignmentsViewProps['role']
  showFilter: boolean
...
function CourseGroup({ course, assignments, role }: CourseGroupProps) {
  if (assignments.length === 0) return null

  return (
    <div className="space-y-4">
      <div className="flex items-baseline gap-4">
        <h3 className="font-serif text-xl text-[#1C1815]">{course.title}</h3>
...
function AssignmentsEmptyState({ selectedCourse }: { selectedCourse: string }) {
  return (
    <div className="py-16 text-center">
      <p className="font-serif text-lg text-[#AFA28F]">No assignments found</p>
...
export function AssignmentsView({ assignments, role }: AssignmentsViewProps) {
  ...
  return (
    <div className="space-y-10">
      <AssignmentsHeader
        role={role}
        showFilter={assignments.length > 0}
        courses={courses}
      ...
      {filteredAssignments.length === 0 && (
        <AssignmentsEmptyState selectedCourse={selectedCourse} />
      )}
    </div>
  )
}
