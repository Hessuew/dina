...
| Purpose | Tool |
| ------- | ---- |
| List candidate issues | `mcp__plugin_linear_linear__list_issues` |
| Get issue details + links | `mcp__plugin_linear_linear__get_issue` |
| Update issue state | `mcp__plugin_linear_linear__save_issue` |
| Resolve workflow status, if available | `mcp__plugin_linear_linear__get_issue_status` |
...
mcp_call_tool({
  ...
  arguments: {
    state: ["In Progress", "In Review", "Todo", "Backlog", "Started"],
...
Before any update, identify the actual workspace state IDs for:
...
Resolution order:
1. Use Linear MCP workflow/status tooling if available, such as `get_issue_status`, to resolve the real state IDs.
2. If no direct status-listing tool exists, inspect existing terminal issues returned by `list_issues` for known completed/canceled state names and IDs.
3. If the state ID is still ambiguous, stop before mutation and ask the user for the exact target state ID.

Do not use placeholder IDs like `done` or `canceled` unless the MCP response proves those are valid IDs in this workspace.
...
- If every PR is merged, plan `stateId: <completed-state-id>`.
- If every PR is closed unmerged, plan `stateId: <canceled-state-id>`.
...
- Apply only the planned issue IDs and state IDs; do not edit title, description, assignee, labels, priority, links, or project metadata.
