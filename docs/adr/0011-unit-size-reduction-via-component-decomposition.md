...
**The lever is fundamentally different from ADR 0010.** CRAP fell via **coverage** (extract
logic → `domain/` → test, so `CRAP = CC² · (1 − cov)³ + CC` collapses). Unit size only falls
via **decomposition**: most oversized functions are long because of **JSX/markup**, which
...
source of truth. No external origin/main checkups should be done.
...
This mirrors ADR 0010: a separate worktree only adds friction, can branch off a stale base,
and splits the changes away from the branch the developer is actually reviewing. Edit the files
in place, run the fast verification loop locally, update this ledger, and stop.
...
layer per the ADR 0010 placement rules; but the bulk of the work is **presentational JSX
...
2. **Route branchy logic to `domain/`** — if the function holds non-trivial logic (initial-
   ...
   `domain/` layer and unit-test it at 100%, reusing the ADR 0010 placement rules:
...
- A committed **`eslint-suppressions.json`** (ESLint 9 bulk suppressions, generated via
  `bunx eslint --suppress-rule max-lines-per-function`) grandfathers the 75 known violations.
- The quality gate runs ESLint, so a new oversized function errors while suppressed ones pass.
- On each paydown, `bunx eslint --prune-suppressions` removes the cleared function's entry.
  `eslint-suppressions.json` is the machine-readable mirror of the ledger below — both shrink
  together. When the file empties, the rule becomes an absolute "no function > 60 LOC."

### Test files are exempt

`max-lines-per-function` is **off for test files**. Long `describe`/`it`/arrange-act-assert
...
- Order by **`line_count` descending** (very-high → low) — the size analogue of ADR 0010's
  Sev → CRAP order. The ledger is pre-sorted.
- **Batch 2–3 same-feature targets per session/commit.** Never batch across features in one
...
Reuse ADR 0010's claim protocol verbatim:
