...
ordered/upcoming lesson queries without changing response shape or retention
behavior. They are additive and safe for the expand/contract release procedure.
...
- The affected post, notification, and course integration suites must remain
  green.
- After hosted data exists, capture `EXPLAIN (ANALYZE, BUFFERS)` for the five
