...
These indexes support the existing cursor pagination and unread read-state
queries without changing response shape or retention behavior. They are
additive and safe for the expand/contract release procedure.
...
- After hosted data exists, capture `EXPLAIN (ANALYZE, BUFFERS)` for the three
