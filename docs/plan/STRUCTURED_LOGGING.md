...
2. Add a shared server logging helper before replacing broad `console.error` and `console.warn` usage. **Done:**
   ...
   raw error messages.
3. Convert high-value server functions first: auth, enrollment, assignment submission, teacher review, admin workflows. **Repository rollout complete through Iteration 190:** assignment submission persistence now emits redacted `assignment_submission_saved` and `assignment_submission_failed` events with request correlation, outcome status, duration, and stable error category fields. Assignment grading now emits a redacted `assignment_grading_completed` event with request correlation, status, duration, assignment, submission, and actor identifiers. Signup and OTP flows now emit the same shape for OTP delivery, account provisioning, rollback, auto-login, resend outcomes, and invitation/profile/OTP persistence failures. The admin invitation-email campaign now emits redacted per-invitation delivery outcomes, campaign summaries, and lock-release failures. Password-reset email delivery and password update outcomes now emit redacted success/failure events with stable categories and provider codes. Enrollment evaluation score, admission-category, and note mutations now emit a shared redacted `enrollment_evaluation_updated` event with request correlation, evaluator/enrollment IDs, action path, status, duration, and field type; unexpected persistence failures use `enrollment_evaluation_persistence`, while unexpected role, reviewer-assignment, and course-team authorization reads reuse `enrollment_evaluation_update_failed` with `enrollment_evaluation_authorization_persistence`; evaluation values and note text are excluded. Avatar and course-thumbnail upload actions now emit request-correlated completion/failure events, while storage URL-signing and old-object cleanup failures use redacted warning events. Student exam submission now emits `exam_attempt_submitted`, `exam_attempt_submission_ignored`, and unexpected `exam_attempt_submission_failed` events with request correlation, attempt/exam/student IDs, outcome status, duration, submission mode, and stable error categories; answers and scores are excluded. The admin WhatsApp campaign now emits redacted per-message delivery outcomes, completion summaries with safe flattened skip counters, and lock-release failures; recipient phone/name data and provider exception text are excluded from structured telemetry. Post/comment notification fan-out now emits redacted `notification_recipients_resolved` / `notification_recipient_resolution_failed` events for safe recipient counts and lookup failures, while `notification_delivery_failed` covers database delivery failures. Root auth bootstrap session/profile persistence failures now reuse redacted `auth_session_lookup_failed` / `auth_profile_lookup_failed` events, and the legacy course-teacher probe emits `course_teacher_check_failed` for unexpected assignment-read failures. Assignment submission and student/teacher assignment-list actor-profile lookups now remain inside their existing redacted failure boundaries with stable `submission_read_persistence` or `assignment_read_persistence` categories. Recipient identities, notification content, and raw persistence details remain excluded; recipient lookup and delivery remain best effort.
   Signup persistence boundaries now emit `signup_invitation_lookup_failed`,
   `signup_profile_lookup_failed`, `signup_otp_update_failed`,
   `signup_otp_attempt_update_failed`, `signup_invitation_acceptance_failed`,
   and `signup_otp_cleanup_failed` with stable read/write categories, safe
   invitation IDs where available, and original-error preservation. Invitation
   tokens, email addresses, passwords, provider messages, and raw database
   details remain excluded; expected invalid, expired, cooldown, and OTP
   validation outcomes stay quiet.
   Unexpected password sign-in provider exceptions now emit the existing
   `login_failed` event at error level with failure status and the stable
   `auth_sign_in` category; credentials and provider exception text remain
   excluded. Assignment-grade persistence failures now emit
   `assignment_grading_failed` with request correlation, safe assignment,
   ...
   addresses, reset tokens, passwords, and provider/database details remain
   excluded, and anonymous, cooldown, invalid, and expired-token outcomes stay
   quiet.
   ...
   categories; unexpected actor-profile preflight failures reuse the same
   action-specific failure events with the stable
   `invitation_actor_profile_read_persistence` category; original repository
   ...
   raw database details remain excluded. Distribution and substitution
   read-preflights also emit `enrollment_distribution_failed` and
   `enrollment_substitution_failed` with stable
   `enrollment_distribution_read_persistence` and
   `enrollment_substitution_read_persistence` categories; original repository
   ...
   Admin email and WhatsApp campaign lock inspection and explicit release now
   emit redacted `*_campaign_locks_loaded`, `*_campaign_locks_load_failed`,
   ...
   Root route authentication bootstrap now uses the same redacted session and
   profile lookup failure events as the explicit auth boundary, preserving
   anonymous `null` context results and adding request correlation to failures.
...
The repository migration is complete through Iteration 190. Future code
...
closeout work is hosted Better Stack/Cloudflare ingestion, dashboard and alert
verification, and evidence collection rather than another unbounded sweep.
