...
| ----- | --------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TM-01 | Broken object-level authorization exposes another student's submission, enrollment, progress, or private content            | Server functions call getCurrentUser; shared authz checks roles, ownership, course teachers, and resource relationships; course detail/catalog services filter drafts to assigned teachers/Admins; student-directory services enforce staff access before reads; post read services require a persisted profile; focused integration tests cover recent calendar, moderation, substitution, teacher-directory, student-directory, course-content, and post-read gaps | Continue route/server-function review whenever a read or mutation is added; add a negative integration case for every new sensitive resource                          |
...
| TM-10 | Dependency or build-tool compromise reaches the deployed bundle or CI                                                       | Weekly Bun audit, Dependabot, patched direct PDF.js and nested Vite/shell-quote/srvx resolutions, and a blocking high/critical threshold with lower-severity report retention are documented in SECURITY.md                                                                                                                                                                                                                                                          | Triage lower-severity findings by reachability, upgrade/replace/isolate/accept decision, owner, and review date; keep the high/critical threshold blocking            |
...
The course catalog service retains draft lessons for Admins and assigned course
teachers, then filters every other teacher's catalog rows to published lessons
before returning the dashboard payload. This closes the parallel list-read
path that could otherwise expose draft lesson data even when course-detail
reads were protected. Integration coverage verifies assigned-teacher draft
access and outsider-teacher filtering.
